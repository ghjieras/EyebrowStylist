...
class ViewController: UIViewController, AVCaptureVideoDataOutputSampleBufferDelegate {
    ...
    func captureOutput(_ output: AVCaptureOutput, didOutput sampleBuffer: CMSampleBuffer, from connection: AVCaptureConnection) {
        ...
        // 設定影像方向
        // 使用 .up 而不是 .upMirrored，因為我們在座標轉換時處理鏡像
        imageOrientation = .up
    ...
    func convertNormalizedRect(_ normalizedRect: CGRect) -> CGRect {
        ...
        let screenRect = CGRect(
            x: (1.0 - normalizedRect.origin.x - normalizedRect.width) * viewSize.width,  // X 軸鏡像
            y: (1.0 - normalizedRect.origin.y - normalizedRect.height) * viewSize.height, // Y 軸翻轉
    ...
    func convertVisionToScreen(_ visionPoint: CGPoint) -> CGPoint {
        ...
        // 基本座標轉換 - 加入 X 軸鏡像來修正方向
        let screenPoint = CGPoint(
            x: (1.0 - visionPoint.x) * viewSize.width,  // X 軸鏡像
            y: (1.0 - visionPoint.y) * viewSize.height  // Y 軸翻轉
        )
